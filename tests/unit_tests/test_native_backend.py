...
@pytest.fixture(params=["tuple", "iter"])
def records(request: pytest.FixtureRequest) -> list[tuple] | list[Iterator]:
    data = [
        ("first column", "two", "three"),
        (1, "a", "foo"),
        (2, "b", "bar"),
        (3, "c", "baz"),
        (4, "d", "qux"),
...
@pytest.fixture
def backend(records: list[tuple] | list[Iterator]) -> NativeBackend:
    return NativeBackend(records)


def test_from_records(records: list[tuple] | list[Iterator]) -> None:
    backend = NativeBackend(records)
    assert backend.column_count == 3
    assert backend.row_count == 5
    assert tuple(backend.columns) == ("first column", "two", "three")


def test_column_content_widths(backend: NativeBackend) -> None:
    assert backend.column_content_widths == [1, 8, 6]


def test_get_row_at(backend: NativeBackend) -> None:
...
def test_get_column_at(backend: NativeBackend) -> None:
...
def test_get_cell_at(backend: NativeBackend) -> None:
...
def test_append_column(backend: NativeBackend) -> None:
    original_table = deepcopy(backend.data)
    ...
    actual = [row[:3] for row in backend.data]
    assert actual == original_table


def test_append_rows(backend: NativeBackend) -> None:
    original_table = deepcopy(backend.data)
    ...
    assert backend.data[0:5] == original_table


def test_drop_row(backend: NativeBackend) -> None:
    backend.drop_row(0)
...
def test_update_cell(backend: NativeBackend) -> None:
...
def test_sort(backend: NativeBackend) -> None:
    original_table = deepcopy(backend.data)
    ...
    assert backend.data == original_table
